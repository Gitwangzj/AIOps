...
structural = tf.contrib.timeseries.StructuralEnsembleRegressor(
      periodicities=24, num_features=1, cycle_num_latent_values=5)

structural.train(input_fn=train_input_fn, steps=1000)
evaluation_input_fn = tf.contrib.timeseries.WholeDatasetInputFn(reader)
evaluation = structural.evaluate(input_fn=evaluation_input_fn, steps=1)

(predictions, ) = tuple(structural.predict(input_fn=tf.contrib.timeseries.predict_continuation_input_fn(
    evaluation, steps=24)))
